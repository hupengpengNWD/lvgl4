...
HAL_StatusTypeDef drv_i2c_touchpad_init(void)
{
    ...
    ux_device_cdc_acm_printf ("gt911 vendor id = %02x %02x %02x %02x\r\n", buf[0], buf[1], buf[2], buf[3]);

    // 产品id
    gt911_get_product_id (buf);
    ux_device_cdc_acm_printf ("gt911 product id = %02x %02x %02x %02x\r\n", buf[0], buf[1], buf[2], buf[3]);

    // 触摸芯片固件版本
    gt911_get_version (buf);
    ux_device_cdc_acm_printf ("version = 0x%x\r\n", buf[0]);

    gt911_get_max_x (buf);
    g_tp_drv.width = (uint16_t) ((buf[1] << 8) | buf[0]);
    ux_device_cdc_acm_printf ("touchpad max x = %d\r\n", g_tp_drv.width);

    gt911_get_max_y (buf);
    g_tp_drv.height = (uint16_t) ((buf[1] << 8) | buf[0]);
    ux_device_cdc_acm_printf ("touchpad max y = %d\r\n", g_tp_drv.height);
...
HAL_StatusTypeDef touchpad_is_touched(void)
{
    ...
    //ux_device_cdc_acm_printf("touch_num: %d\r\n", touch_num);
